    ...
    with open(filename, 'r') as file:
        ...
        for line in lines:
            ...
            if match:
                ...
                # Add the subset columns if they exist in the elements list
                for i, col in enumerate(subset):
                    if i in index and len(elements) > index[i]:
                        node[col] = elements[index[i]]
        ...
        logging.info(f"Processed area file: {filename}")
...
def call_Preprocess(file_name, runName):
    ...
    with open(file_name, 'r') as file:
        ...
        positions = [data.index(column) if column in data else None for column in subset]
    
    # Call the parse_data function to parse the file
