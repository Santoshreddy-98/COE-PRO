...
function PreSynthesisReport() {
  const [checklistItems, setChecklistItems] = useState({ pdDev: null, pdLead: null });
  ...
  const questionKeys = pdDevQuestionKeys.length > 0 ? pdDevQuestionKeys : pdLeadQuestionKeys;
        ...
        <table className="report-table">
          ...
          <tbody>
            {questionKeys.map((questionKey) => (
              <tr key={questionKey}>
                <td>{pdDevItem?.questions[questionKey] || pdLeadItem?.questions[questionKey]}</td>
                <td>{data || 'NA'}</td>
                <td>{pdDevData[`${questionKey}`.replace('question', 'pdDev')] || 'NA'}</td>
                <td>{devCommentsData[`${questionKey}`.replace('question', 'devComments')] || 'NA'}</td>
                <td>{pdLeadData[`${questionKey}`.replace('question', 'pdLead')] || 'NA'}</td>
                <td>{pdLeadComments[`${questionKey}`.replace('question', 'pdLeadComments')] || 'NA'}</td>
              </tr>
            ))}
          </tbody>
        </table>
