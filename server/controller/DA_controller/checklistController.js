...
exports.createChecklistItem = async (req, res) => {
  try {
    ...
    const { data, pdDev, devComments, pdLead, pdLeadComments } =
      req.body.answers;
    ...
    const pdDevObj =
      userRole === "PD Dev" ? new Map(Object.entries(pdDev)) : undefined;

    const devCommentsObj =
      userRole === "PD Dev" ? new Map(Object.entries(devComments)) : undefined;

    const pdLeadObj =
      userRole === "PD Lead" ? new Map(Object.entries(pdLead)) : undefined;

    const pdLeadCommentsObj =
      userRole === "PD Lead"
        ? new Map(Object.entries(pdLeadComments))
        : undefined;
